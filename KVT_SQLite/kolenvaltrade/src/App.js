...
const jwtDecode = require('jwt-decode');//можно ли на фронте раскодировать токен для информации о юзере?
...
const actionDeleteAd = (obj) => {
  fetch('/deleteAd', {
    ...
    .then(json => {
      if(json != 'cannot delete what does not belong to you' && json != 'unauthorized'){
      ...
      return(
        <button class="btn btn-warning changeUser" onClick = {() => {   
          fetch('/getUser', {
            ...
            .then(json => {
              this.setState({flag: true, login: json.login, password: '', name: json.name, phone: json.phones, loginValid: true,
...
class SearchForm extends React.Component{
  ...
  render(){
    ...
    let {params, className, ...obj} = this.state
    let selects = []
    for(let param in params){
      let options = []
  ...
  render(){
      ...
      for(let ad of this.props.ads){
        ads.push(
          <div class='ad' id={ad._id}>
            <img class = "preview" src={ad.photos[0]? ad.photos[0].name: null} alt='Машина'></img>
            <div>
              <h4>{ad.manufactor.name} {ad.model.name}</h4> <h5>{ad.price}$</h5>
              <h4>{ad.region.name} область, город {ad.city.name}</h4>
              <p>
    ...
    if(this.props.type == 'ads'){
      for(let ad of this.props.ads){
        ads.push(
          <div class='ad' id={ad._id}>
            <img class = "preview" src={ad.photos[0]? ad.photos[0].name: null} class = "preview"  alt='Машина'></img>
            <div>
              <h4>{ad.manufactor.name} {ad.model.name}</h4> <h5>{ad.price}$</h5>
              <h4>{ad.region.name} область, город {ad.city.name}</h4>
              <p>
      ...
      <div class='advertisement'>
        <AliceCarousel 
          items={this.state.ad.photos.map(photo => {
            return(
              <img class="carousel" src = {photo.name}/>
            )
          })}

        />
        <h4>{this.state.ad.manufactor.name} {this.state.ad.model.name}</h4> <h5>{this.state.ad.price}$</h5>
        <h4>{this.state.ad.region.name} область, город {this.state.ad.city.name}</h4>
        <h4>seller name {this.state.ad.user.name} phone {this.state.ad.user.phones}</h4>
        <p>
